...
module Rocket
  class CLI < Optitron::CLI
    ...
    opt "log",     :short_name => "l", :default => "/var/log/rocket/server.log", :desc => "Path to log file"
    def start
      Rocket.load_settings(params.delete('config'), params)
    ...
    def stop
      Rocket.load_settings(params.delete('config'), params)
      Rocket::Server.new(Rocket.settings).kill!
    end
